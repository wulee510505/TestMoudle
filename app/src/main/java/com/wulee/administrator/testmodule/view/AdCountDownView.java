...
/**
 * Created by wulee on 2016/10/12 15:52
 */

public class AdCountDownView extends View {
    private static final String TAG = AdCountDownView.class.getSimpleName();
    ...
    public AdCountDownView(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.AdCountDownView);
        backgroundColor = ta.getColor(R.styleable.AdCountDownView_background_color, BACKGROUND_COLOR);
        borderWidth = ta.getDimension(R.styleable.AdCountDownView_border_width, BORDER_WIDTH);
        borderColor = ta.getColor(R.styleable.AdCountDownView_border_color, BORDER_COLOR);
        text = ta.getString(R.styleable.AdCountDownView_ad_text);
        if (text == null) {
            text = TEXT;
        }
        textSize = ta.getDimension(R.styleable.AdCountDownView_ad_text_size, TEXT_SIZE);
        textColor = ta.getColor(R.styleable.AdCountDownView_ad_text_color, TEXT_COLOR);
        ta.recycle();
        init();
    }
...
//       canvas.drawText("稍等片刻", width / 2, height / 2 - textPaint.descent() + textPaint.getTextSize() / 2, textPaint);
    ...
    public void start() {
        ...
        countDownTimer = new CountDownTimer(5000, 36) { //从5s开始以每36ms为单位倒计时
            @Override
            public void onTick(long millisUntilFinished) {
                progress = ((5000 - millisUntilFinished) / 3600f) * 360;
